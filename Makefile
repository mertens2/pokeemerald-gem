...
ROM_NAME := Pokémon_Bismuto.gba
ELF_NAME := $(ROM_NAME:.gba=.elf)
MAP_NAME := $(ROM_NAME:.gba=.map)
OBJ_DIR_NAME := build/emerald

MODERN_ROM_NAME := $(ROM_NAME)
MODERN_ELF_NAME := $(MODERN_ROM_NAME:.gba=.elf)
MODERN_MAP_NAME := $(MODERN_ROM_NAME:.gba=.map)
MODERN_OBJ_DIR_NAME := $(OBJ_DIR_NAME)
...
override CFLAGS += -mthumb-interwork -Wimplicit --print-memory-usage -fshort-enums -Wparentheses -Werror -O2 -fhex-asm -g
ROM := $(ROM_NAME)
...
CPPFLAGS := -iquote include -iquote -Wno-trigraphs -DMODERN=1
ifneq ($(MODERN),1)
...
RULES_NO_SCAN += libagbsyscall clean clean-assets tidy tidymodern clean-generated
...
ifeq (,$(MAKECMDGOALS))
  ...
  ifeq (,$(filter-out clean tidy tools clean-assets clean-tools $(TOOLDIRS) tidymodern tidynonmodern libagbsyscall,$(MAKECMDGOALS)))
    SCAN_DEPS ?= 0
  else
    SCAN_DEPS ?= 1
  endif
...
$(C_BUILDDIR)/librfu_intr.o: CFLAGS := -O2 -march=armv4t -mthumb-interwork -quiet
else
$(C_BUILDDIR)/librfu_intr.o: CFLAGS := -march=armv4t -mthumb-interwork -O2 -mabi=apcs-gnu -mtune=arm7tdmi -march=armv4t -fno-toplevel-reorder -Wno-pointer-to-int-cast
endif
