...
{
    bool8 result = FALSE;
    if (gTextFlags.autoScroll != 0 || AAndBCanSkip(OPTIONS_TEXT_SKIP_NOCLOSE))
    {
        ...
        do {
			...
			case EXT_CTRL_CODE_BEGIN:
				...
				case EXT_CTRL_CODE_ENG:
					...
					return RENDER_REPEAT;
				// case EXT_CTRL_CODE_SHAKE_SCREEN:
					// return RENDER_REPEAT;
				}
				break;
			...
			repeats--;
        } while ((AAndBCanSkip(OPTIONS_TEXT_SKIP_NOSCROLL)) && repeats > 0);
        return RENDER_PRINT;
...
u8 RenderTextHandleBold(u8 *pixels, u8 fontId, u8 *str)
{
    ...
    int temp2;
    u8 colorBackup[3];
    u8 fgColor, bgColor, pans, delay, numShakes;
    ...
    do
    {
        ...
        case EXT_CTRL_CODE_BEGIN:
			...
			case EXT_CTRL_CODE_SHAKE_SCREEN:
                ...
                break;
			// case EXT_CTRL_CODE_SHAKE_SCREEN:
				// pans = strLocal[strPos++];
				// delay = strLocal[strPos++];
				// numShakes = strLocal[strPos++];
				// ShakeCameraScript(pans, pans, delay, numShakes);
				// continue;
            case EXT_CTRL_CODE_RESET_SIZE:
            case EXT_CTRL_CODE_PAUSE_UNTIL_PRESS:
            case EXT_CTRL_CODE_WAIT_SE:
